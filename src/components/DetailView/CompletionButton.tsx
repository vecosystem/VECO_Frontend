...
interface CompletionButtonProps {
  ...
  isSaving: boolean; // 상위에서 내려주는 저장 중 상태
...
const CompletionButton = ({
  ...
  // 버튼 비활성화 조건:
  // (1) 제목 입력이 안 됐을 때 & 작성 완료 상태가 아닐 때
  // (2) 저장 중일 때
  const isDisabled = (!isTitleFilled && !isCompleted) || isSaving;

  const buttonLabel = isSaving ? '저장 중…' : isCompleted ? '수정하기' : '작성 완료';
  ...
  return (
    <div className="flex items-end justify-end fixed bottom-[5.3rem] right-[6.4rem] z-20">
      ...
      >
        {/* 버튼 아이콘 */}
        {isSaving ? (
          // 저장 중에는 일시적으로 스피너가 뜨게
          <div className="size-4 animate-spin rounded-full border-[2px] border-white/80 border-t-transparent" />
        ) : (
          <img
